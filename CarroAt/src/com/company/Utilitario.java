...
public class Utilitario extends App {
    public Utilitario(String fabricante, String modelo, int ano_Fabricacao, double preco) {
        super(fabricante, modelo, ano_Fabricacao, preco);
    }

    @Override
    void ConsultarDados() {
        JOptionPane.showMessageDialog(null, "\nFabricante do carro: " + getFabricante() +
    ...
    //Depreciação Utilitario 0,5%
    @Override
    void Depreciacao() {
        int tempo_Uso;
        double preco_Final, preco;
        tempo_Uso = 2021 - getAno_Fabricacao();
        preco_Final = getPreco() * Math.pow(1 + (-0.005), tempo_Uso);
